...
class AppDelegate: UIResponder, UIApplicationDelegate, SSASideMenuDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool
    {
        ...
        let isLoggedIn = NSUserDefaults.standardUserDefaults().objectForKey("is_active") as? Bool
        if(isLoggedIn == true){
            print("Already logged in")
            UINavigationBar.appearance().barTintColor = UIColor(red: 0.91, green: 0.10, blue: 0.10, alpha: 1.0)
            UINavigationBar.appearance().titleTextAttributes = [NSFontAttributeName: UIFont(name: "Gotham-Medium", size: 18)!,  NSForegroundColorAttributeName: UIColor.whiteColor()]
            UINavigationBar.appearance().barStyle = UIBarStyle.BlackTranslucent
            UINavigationBar.appearance().translucent = false
            UINavigationBar.appearance().tintColor = UIColor.whiteColor()
            
            let viewController:UIViewController = UIStoryboard(name: "Main", bundle: nil).instantiateViewControllerWithIdentifier("HomePageViewController")
            let nav = UINavigationController(rootViewController: viewController)
            
            let sideMenu = SSASideMenu(contentViewController:nav, leftMenuViewController: UIStoryboard(name: "Main", bundle: nil).instantiateViewControllerWithIdentifier("LeftMenuViewController"))
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "com.webwerks.NeoSTORE" in the application's documents Application Support directory.
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("NeoSTORE", withExtension: "momd")!
